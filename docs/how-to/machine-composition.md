...
> **Tier 2 — experimental.** `InvokeMachine` / `WithChildMachine` may change in a future v1.x minor. Pin a specific minor for production use. See [API Stability Tiers](../reference/stability.md). Overlaps with the actor `Spawn` API; prefer this builder when typed child composition is enough.
